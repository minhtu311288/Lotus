...
export default function Profile() {
    ...
    useEffect( async () => {
        const AuthStr = 'Bearer '.concat(cookies.jwt);
        const dataFetch = await CallAPIGET('/getprofile', { headers: { "Authorization": AuthStr } })
        setData(dataFetch)
    }, [])
        ...
        <Layout>
            <div className="max-w-4xl flex items-center h-auto lg:h-screen flex-wrap mx-auto my-32 lg:my-0">
                ...
                <div className="w-full lg:w-2/5">
                    <img src={data?.avatar} className="rounded-none lg:rounded-lg shadow-2xl hidden lg:block" />
                </div>
            </div>
        </Layout>
    )
